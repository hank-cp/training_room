...
$.jgrid.extend({
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(t.p.datatype === 'local') {
						...
						t.p.data.push($.extend(lcdata, data));
                        lcdata = {};
					}
				}
	...
	remapColumns : function(permutation, updateCells, keepHeader) {
		...
		function resortRows(parent, clobj) {
			var rows = $(">tr", parent)
			if (clobj) rows = rows.filter(clobj)
            rows.each(function() {
				var row = this;
		...
		var $headerRow = $("thead:first", ts.grid.hDiv)
		resortRows($headerRow, keepHeader && ":not(.ui-jqgrid-labels)");

		if ($headerRow.has('.ui-th-column-header').length > 0) {
			var $secondLevelHeader = $(">tr.jqg-second-row-header >th.ui-th-column-header", $headerRow)
			var $thirdLevelHeader = $(">tr.jqg-third-row-header", $headerRow)

			$secondLevelHeader.each(function() {
				$.each($(this).data('coverCol'), function (i, colId) {
					$thirdLevelHeader.append($thirdLevelHeader.find("#" + ts.p.id + "_" + colId.replaceAll('/', '\\/').replaceAll('\\.', '\\.')))
				})
			})
		}
	...
	editCell : function (iRow,iCol, ed, event){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
				...
				if( $.inArray(cm.edittype, ['text','textarea','password','select', 'number']) > -1) {
		...
		return this.each(function(){
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader.data('coverCol').push(th.id.replace(/^jqgh_/, "").replace(ts.p.id+"_",""))
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
						...
						if(cm[i].editable===true) {
							...
							if( $.inArray(cm[i].edittype, ['text','textarea','password','select','number']) > -1) {
								$(elc).addClass( inpclass );
							}
				...
				if ( nm !== 'cb' && nm !== 'subgrid' && cm.editable===true && nm !== 'rn' && !$(this).hasClass('not-editable-cell')) {
					switch (cm.edittype) {
						...
						case 'select':
							...
							if((cm.formatter && cm.formatter === 'select') || cm.editoptions) { tmp2={}; }
                            elem = $("select",this);
							break;
						case 'custom' :
							try {
		...
		return this.each(function (){
			...
			var sortable_opts = {
				...
				"update": function(event, ui) {
					...
					var permutation = [];
					th.each(function() {
						if ($(this).hasClass('ui-th-column-header')) {
							$.each($(this).data('coverCol'), function(i, colId) {
								permutation.push(cmMap[colId]);
							})
						} else {
					...
					if ($.isFunction(ts.p.sortable.update)) {
						ts.p.sortable.update($(ts), permutation);
					}
				}
			};
				...
				while(j < dlen) {
					...
					for(i = 0; i < cmlen; i++) {
						if(cm[i].exportcol) {
                            ...
                            tmp[k] = $.jgrid.formatCellCsv( row[cm[i].name], p );
							k++;
						}
					}
